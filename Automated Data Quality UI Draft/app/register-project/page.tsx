        ...
        <div className={local.layerGrid}>{layerOrder.map((layer) => {
          ...
          return <section className={local.layerGroup} key={layer}><header className={local.layerHead}><div><span>{layer}</span><small>{layerDescriptions[layer]}</small></div><strong>{layerAssets.length}</strong></header>
            {connectionIds.map((connectionId) => {
              ...
              return <div className={local.technologyGroup} key={connectionId}><header><div className={local.typeIcon}>{profile ? connectionLabels[profile.kind].slice(0, 2).toUpperCase() : "?"}</div><div><strong>{profile?.name || connectionId}</strong><small>{profile ? connectionLabels[profile.kind] : "Unknown connection"} · {tests[connectionId]?.status ?? "NOT TESTED"}</small></div></header>
                {types.map((type) => {
                  ...
                  return <details className={local.assetTypeGroup} key={type} open={types.length <= 3}><summary><span>{type}</span><strong>{typeAssets.length}</strong></summary><div className={local.assetCards}>{typeAssets.map((asset) => {
                    ...
                    return <details className={local.assetCard} key={asset.id}><summary><input type="checkbox" aria-label={`Select ${asset.name}`} checked={selected} onClick={(event) => event.stopPropagation()} onChange={(event) => onSelect(asset.id, event.target.checked)} /><div><strong>{asset.name}</strong><small>{[asset.catalog, asset.schema].filter(Boolean).join(" / ") || asset.detail || "No namespace"}</small></div><span className={local.proposedBadge}>{asset.roleStatus ?? "PROPOSED"} {asset.proposedLayer ?? "ROLE"}</span></summary><div className={local.assetEvidence}>
                      <div><span>Project</span><strong>{project.name}</strong></div><div><span>Environment</span><strong>{environment}</strong></div><div><span>Connection</span><strong>{profile?.name || connectionId}</strong></div><div><span>Connection test</span><strong>{tests[connectionId]?.status ?? "NOT TESTED"}</strong></div><div><span>Selection status</span><strong>{selected ? "SELECTED" : "NOT SELECTED"}</strong></div>
...
function ProjectOnboardingPage() {
  ...
  if (phase === "overview") content = <div className={local.overviewStack}>
    ...
    {!connections.length ? <div className={local.empty}><strong>No connections configured</strong><p>Add one manually or import the detected runtime. Every field remains editable before testing.</p></div> : <div className={local.connectionTableWrap} role="region" aria-label="Configured connections" tabIndex={0}><table className={local.connectionTable}><thead><tr><th>Connection</th><th>Adapter</th><th>Environment</th><th>Test status</th><th>Last tested</th><th>Actions</th></tr></thead><tbody>{connections.map((profile) => { const result = tests[profile.id]; return <tr key={profile.id}><td><button className={local.connectionName} onClick={() => setEditing(profile)}><strong>{profile.name}</strong><small>{endpoint(profile)}</small></button>{result?.status === "FAIL" && <span className={local.failureReason}>{result.detail}</span>}</td><td><strong>{connectionLabels[profile.kind]}</strong><small className={local.adapterDetail}>{adapterCapabilities(profile)}</small></td><td>{profile.environment}</td><td><span className={`${local.status} ${statusClass(result?.status)}`}>{result?.status ?? "NOT TESTED"}</span></td><td>{result ? new Date(result.testedAt).toLocaleString() : "Not tested"}</td><td><div className={local.tableActions}><button className={styles.quiet} onClick={() => setEditing(profile)}>Edit</button><button className={styles.secondary} disabled={busy !== null} onClick={() => void testConnection(profile)}>{busy === `test:${profile.id}` ? "Testing…" : "Test"}</button><button className={styles.secondary} disabled={testResultNotReady(result) || busy !== null} title={testResultNotReady(result) ? "Test this connection successfully first" : "Run discovery for this connection"} onClick={() => { setPhase("discovery"); void runDiscovery(profile); }}>{busy === `discover:${profile.id}` ? "Discovering…" : "Discover"}</button><button className={styles.quiet} onClick={() => void removeProfile(profile.id)}>Remove</button></div></td></tr>; })}</tbody></table></div>}
  </section></div>;
      ...
      {!connections.length ? <div className={local.empty}><strong>No connections configured</strong><p>Configure and test a connection before starting discovery.</p></div> : <details className={local.diagnosticDetails}><summary>Connector diagnostics <span>{connections.length} connectors · expand to test or rediscover</span></summary><div className={local.discoveryConnections}>{connections.map((profile) => {
        ...
        return <article key={profile.id} className={local.discoveryConnection}>
          <header className={local.discoveryCardHead}>
            ...
            <div className={local.discoveryStatusRow}><span className={`${local.status} ${statusClass(test?.status)}`}>{test?.status ?? "NOT TESTED"}</span><span className={`${local.status} ${statusClass(discovery?.status)}`}>{discovery?.status ?? "NOT RUN"}</span></div>
          </header>
