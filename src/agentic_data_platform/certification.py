...
def coco_certification_status(root: str | Path) -> dict[str, Any]:
    ...
    A checked-in/local report is treated as historical evidence only when its head is
    explicitly supplied by the producing workflow. Operator surfaces therefore show
    implementation coverage separately from exact-head CI evidence.
    """
