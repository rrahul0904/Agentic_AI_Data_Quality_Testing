...
RGA synthetic domain YAML
        ...
        |
        +--> Power BI / Excel governed Microsoft consumer contract (account-gated)
        |       +--> Power BI generated DAX + Execute DAX Queries evidence
        |       +--> Excel live/XMLA evidence
        |
...
`generate_microsoft_consumer_pack.py` generates a shared Microsoft consumer contract, XMLA setup template, and parity checklist. Endpoint availability is modeled as `account_entitlement_or_feature_gate_required`: the operator must detect whether the Snowflake Semantic Views XMLA Endpoint powered by AtScale is actually enabled/entitled in the target account instead of assuming a permanent preview state or universal availability.

For Power BI, every canonical parity case also generates a `.powerbi.dax` query. `capture_power_bi_parity_evidence.py` can execute those governed DAX queries against a supported Power BI semantic model through the Execute DAX Queries API, consume the Apache Arrow response, detect Arrow error rowsets, normalize result columns, and write standard parity evidence. The live API path is fail-closed without workspace/dataset identifiers, authentication, and explicit confirmation.

Excel remains an independent governed surface. Power BI REST evidence does not certify Excel; Excel must connect through the governed live/XMLA path and produce its own workbook/Pivot/query evidence.
...
`generate_airflow_dag.py` generates the `rga_synthetic_semantic_pipeline` DAG. The flow covers generation -> validation -> contracts -> Snowflake bootstrap -> RAW load -> dbt build -> Semantic View verification, with default-off branches for Semantic View deployment, CDC correction/late-arrival validation, live concurrency benchmarking, and governed physical-optimization analysis. Performance/optimization stages bind only to benchmark files generated by the current sweep; read-only diagnostics remain optional and there is no automatic physical-mutation task. The DAG has no schedule, no catchup, and one active run.
...
**Not yet claimed:** live Snowflake object creation and RAW/CDC application, live dbt execution, server-side Semantic View deployment verification, live benchmark numbers, live Cortex Agent/MCP execution, live Power BI API/XMLA parity from a target semantic model, or live Excel XMLA evidence. Those require the target Snowflake/Microsoft environments, credentials, permissions, and account capabilities/entitlements.
...
- live Power BI Execute DAX Queries and/or XMLA parity evidence against the target governed semantic model;
- live Excel XMLA/Pivot evidence under the same declared cross-system security context;
- target-account detection/certification of Snowflake Semantic Views XMLA endpoint availability.
