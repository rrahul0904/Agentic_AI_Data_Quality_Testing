      ...
      <section className="workspace">
        ...
        {!overview && active === "Overview" ? <Loading /> : (
          <>
            ...
            {active === "Teams" && <CapabilityWorkbench domain="teams" title="Persistent Agent Teams" eyebrow="ROLES · TOOL SCOPES · DEPENDENCY WAVES" description="Create persistent teammate teams, bind member roles/tool scopes/budgets/verification contracts, plan dependency DAGs, and execute approved parallel waves through real AgentRuntime subagents." defaultArgs={{ team_database: ".ade/teammates.db", teammate_database: ".ade/teammates.db" }} />}
            {active === "Hosted Runner" && <CapabilityWorkbench domain="runner" title="Hosted ADE Runner" eyebrow="DURABLE JOBS · LEASES · WORKSPACE ISOLATION" description="Submit and execute durable ADE jobs with actor/interaction mode, approval state, explicit filesystem/network/resource isolation contracts, readiness checks, cancellation, and runner heartbeats." defaultArgs={{ runner_database: ".ade/hosted-runner.db" }} />}
            ...
            {active === "Apps" && <CapabilityWorkbench domain="app" title="Application Lifecycle" eyebrow="SCAFFOLD · PREVIEW · VERIFY · DEPLOY · ROLLBACK" description="Build generic Python/Node/static applications or Snowflake Streamlit/App Runtime projects, materialize fingerprint-bound scaffolds, run isolated previews, verify health, deploy to Docker/Kubernetes/Snowflake, and plan approved rollback." />}
            {active === "ML Registry" && <CapabilityWorkbench domain="ml" title="Agentic ML / Model Registry" eyebrow="PREPARE · COMPARE · SELECT · REGISTER · VERIFY" description="Run portable multi-candidate ML workflows with lineage/data/cost evidence and verified local artifacts, alongside Snowpark ML training and Snowflake Model Registry lifecycle operations." />}
            ...
            {active === "Desktop / IDE" && <CapabilityWorkbench domain="ide" title="Desktop / IDE Bridge" eyebrow="VSCODE · SESSIONS · CHECKPOINTS · CONTEXT · EDIT PLANS" description="Generate a project-scoped VS Code bridge with session browsing, checkpoint review, exact file context and selected-edit planning. Mutation approval/application remains on the shared ToolRegistry boundary." />}
            {active === "Rules & Context" && <CapabilityWorkbench domain="rules" title="Rules & Context Policy" eyebrow="GLOBAL · PROJECT · PATH-SCOPED INSTRUCTIONS" description="Inspect and manage deterministic global/project/path-scoped instruction rules. Context pin/exclude and deterministic compaction remain available in the Advanced Workbench under the same interaction-mode policy." defaultArgs={{ project: "." }} />}
            {active === "Advanced Workbench" && <CapabilityWorkbench domain="advanced" title="Governed Advanced Workbench" eyebrow="AGENT · PLAN · ASK · EDIT · CODE" description="Exercise interaction-mode enforcement through the same ToolRegistry for immutable plans, hash-bound whole-file/region edits, sandboxed shell and Python REPL, deterministic/semantic code search, Git, context pin/exclude/compaction, audited research, retrieval, custom agents, SQL, charts, anomaly/forecasting, documents, SDK, account administration, and GPU controls." defaultArgs={{ workspace: ".", environment: "dev" }} />}
            {active === "Cost / FinOps" && <DomainView title="Cost / FinOps" eyebrow="EVIDENCE-BACKED COST INTELLIGENCE" endpoint="/api/v1/finops/report" />}
            {active === "Governance / PII" && <CapabilityWorkbench domain="governance" title="Governance / Permissions" eyebrow="PII · RBAC · LEAST PRIVILEGE · VERIFIED CHANGES" description="Audit RBAC and sensitive-data exposure, plan least-privilege permission changes with blast-radius evidence, and execute approved changes only through connectors that explicitly support governed mutation followed by independent verification." />}
